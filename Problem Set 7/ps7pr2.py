...
def inner_grid(height, width, digit):
    """ creates and returns a 2-D list of height rows and width columns 
        in which the inner values are all equal to the given digit
        while all the outer values are 0
        input height, width, digit are integers.
    """
...
def copy(grid):
    """ creates and returns a copy of an actual list 
        not a copy of the reference
    """
    copy_grid = create_grid(len(grid), len(grid[0]))
    for r in range(len(grid)):
        for c in range(len(grid[0])):
            copy_grid[r][c] = grid[r][c]
    return copy_grid


def add_one(grid):
    """ returns a 2D list in which the values of the given list
...
def negate_bits(grid):
    """ negates a 2D list consisting of binary digits
    """
    for r in range(len(grid)):
        for c in range(len(grid[0])):
            if grid[r][c] == 0:
